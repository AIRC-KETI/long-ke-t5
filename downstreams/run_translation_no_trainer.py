...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Keyword arguments for generate function. This argument will be  passed to ``model.generate``, which is used during ``evaluate`` and ``predict``.",
    )
    ...
    parser.add_argument(
        "--eval_on_last_epoch",
        action="store_true",
        help="Evaluate performance on the last epoch.",
    )
    parser.add_argument(
        "--save_samples",
        action="store_true",
        help="Save samples that are generated by the model using the evaluation split.",
    )
    parser.add_argument(
        "--report_to",
        type=str,
...
def main():
    ...
    metric_dict = {k:evaluate.load(k) for k in ["sacrebleu", "bleu", "rouge"]}
    eval_metric = None
    generated_samples = []
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        do_eval = (epoch == args.num_train_epochs - 1) if args.eval_on_last_epoch else True
        ...
        if do_eval:
            ...
            for step, batch in enumerate(eval_dataloader):
                ...
                with torch.no_grad():
                    ...
                    if args.save_samples:
                        input_tokens = accelerator.pad_across_processes(batch["input_ids"], dim=1, pad_index=tokenizer.pad_token_id)
                        input_tokens = accelerator.gather(input_tokens)
                        input_tokens = input_tokens.cpu().numpy()
                        decoded_inputs = tokenizer.batch_decode(input_tokens, skip_special_tokens=True)
                        generated_samples.extend(
                            [{
                                "inputs": x,
                                "preds": y,
                                "labels": z
                            } for x, y, z in zip(decoded_inputs, decoded_preds, decoded_labels)]
                        )
    ...
    if args.output_dir is not None:
        ...
        if args.save_samples and len(generated_samples) > 0:
                with open(os.path.join(args.output_dir, "generated_samples.json"), "w") as f:
